        ...
        getDefaultPollText() {
            if (this.endAppealSetting?.QuestionTextTranslations?.length) {
                for (const translation of this.endAppealSetting?.QuestionTextTranslations) {
                    ...
                    if (translation.LanguageCode === this.client.LanguageCode) {
                        return translation.Translation
                    }
                }
            }
            if (this.endAppealSetting?.QuestionText) {
                return this.endAppealSetting?.QuestionText
            }
            return "Пожалуйста, оцените качество консультации"
        },
    },
};
</script>
...
<template lang="pug">
    .rating
        ...
        .pending(v-if="rating.State === 'poll' && start")
